...
- **`assets/crop-*.svg`** — the crop circles in the background. Regenerate with `dev/mkfield.py`.
  Applied as CSS **masks**, so each takes its colour from `--signal` and one file serves both
  themes. A section opts in with `.field` plus a `.f-*` modifier that supplies the source, size and
  corner; `crop-cipher.svg` is wide rather than round, so it also sets `--f-ar`.
